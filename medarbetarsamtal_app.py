...
# Safe import for Pillow and Google Generative AI
try:
...
# Custom Dark / Modern Styling with Custom Scrollbars
st.markdown("""
<style>
    /* Dark Theme Background */
    .stApp {
        background: linear-gradient(135deg, #0B0F19 0%, #111827 50%, #1E1035 100%) !important;
        background-attachment: fixed !important;
        color: #F8FAFC !important;
    }

    /* Scrollbar styling for entire app & text areas */
    ::-webkit-scrollbar {
        width: 10px;
        height: 10px;
    }
    ...
    /* Streamlit Text Area Scrollable Box */
    .stTextArea textarea {
        color: #F8FAFC !important;
...
def call_gemini_with_fallback(contents, system_instruction=None):
    """Dynamically tries available Gemini models to avoid 404 error."""
    if not (HAS_GENAI and GEMINI_CONFIGURED):
        return None

    # Preferred model list in order
    candidate_models = [
        'gemini-1.5-flash',
    ...
    # Check available models if possible
    try:
        available = [m.name.replace('models/', '') for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        if available:
            # Reorder candidate_models based on available
...
def clean_ai_output(text):
    """Strips meta-reasoning, roles, transcription logs, and thoughts, returning ONLY clean Swedish text."""
    if not text:
        return ""
    ...
    ignore_keywords = [
        "role:", "task:", "topic:", "input:", "constraints:", "note:", "notes:",
        "analysis:", "transcription:", "re-evaluating", "correction", "top part",
        "main section", "drafting", "here is", "här är", "point ", "punkt ",
        "formal/professional", "no headings", "ready to be", "keep all facts",
        "transcribe everything", "partially visible", "re-evaluating", "correction/refinement",
        "looking closer", "let's look at", "could it be", "i will interpret"
    ]
    ...
    for line in lines:
        ...
        # Skip reasoning/prompt-mirroring lines
        if any(kw in line_lower for kw in ignore_keywords):
            continue
            
        # Clean bullet markdown if present
        if line_str.startswith("*") or line_str.startswith("-"):
            line_str = re.sub(r"^[*\-\s]+", "", line_str)
            
        # Strip quote marks and bold formatting markers if wrapping entire sentence
        line_str = line_str.strip("\"'`* ")
        
        if len(line_str) > 10:
            valid_paragraphs.append(line_str)
...
# ---------------------------------------------------------
SYSTEM_PROMPT = """Du är en erfaren rektor och chef i svensk skola.
Din ENDA uppgift är att omvandla handskrivna/råa anteckningar från medarbetarsamtal till tjänstemannamässig, formell och professionell svenska lämplig för kommunens skoldokumentation.

ABSOLUTA FÖRBUD:
1. Svara ALDRIG med din roll, uppgift, instruktioner, tänkande eller begränsningar.
2. Inkludera ALDRIG din analys, dina tankar, din transkriberingsprocess eller tolkningar av ord i svaret.
3. Inkludera ALDRIG rubriker, engelska ord, punktlistor, citationstecken eller inledande hälsningar.
4. Generera ENBART den slutgiltiga, färdiga texten i rena svenskspråkiga stycken som kan klistras direkt in i en Word-mall."""

def transform_notes_with_ai(raw_text=None, image_input=None, area_choice="Arbetssituation – nuläge"):
    """Transforms raw text or image into formal Swedish text for official school documentation."""
    
    prompt = f"""Omvandla följande anteckningar till tjänstemannamässig svenska för skolans dokumentation.
Område: {area_choice}

Skriv BARA den slutgiltiga texten. Ingen analys, inga tankar, inga rubriker."""
    ...
    if HAS_GENAI and GEMINI_CONFIGURED:
        ...
        if contents:
            raw_response = call_gemini_with_fallback(contents, system_instruction=SYSTEM_PROMPT)
            if raw_response:
    ...
    source_str = raw_text if raw_text else "God arbetssituation med bra trivsel i laget."
...
Svara EXAKT i följande tre rader utan övriga kommentarer:
...
def main():
    ...
    with tab1:
        ...
        st.info("💡 Ladda upp ett foto på dina handskrivna anteckningar ELLER klistra in råtext. Appen tolkar bilden/texten och genererar BARA den färdiga officiella texten — redo att klistras rakt in i din Word-mall.")
        ...
        with col_input:
            ...
            uploaded_img = st.file_uploader(
                ...
                type=["png", "jpg", "jpeg"],
                help="Ta en bild med mobilen och ladda upp. AI tolkar handstilen direkt."
            )
            
            pil_image = None
            if uploaded_img and HAS_PIL:
                try:
                    pil_image = Image.open(uploaded_img)
                    st.image(pil_image, caption="Uppladdad anteckningsbild", use_column_width=True)
                except Exception as e:
                    st.error(f"Kunde inte läsa bilden: {e}")
            
            with st.expander("✏️ Eller klistra in text manuellt (om du inte använder foto)"):
        ...
        with col_output:
            st.markdown("### 📤 Färdigformaterad text (Rullningsbar box)")
            st.caption("Texten visas i en scrollbar ruta utan analyser eller tänkande — redo för copy-paste:")
            
            if btn_transform or uploaded_img or raw_text_input:
                with st.spinner("Tolkar anteckningar och formaterar officiell text..."):
                    if pil_image:
                        result_text = transform_notes_with_ai(image_input=pil_image, area_choice=section_choice)
                    else:
                        result_text = transform_notes_with_ai(raw_text=raw_text_input, area_choice=section_choice)
                
                st.text_area(
                    "Slutgiltig officiell text (Kopiera härifrån):",
                    value=result_text,
                    height=320,
                    help="Ruta med rullningslist till höger om texten är lång."
                )
                st.success("✅ Den slutgiltiga texten är färdig! Inga interna analyser eller instruktioner ingår.")
    ...
    with tab4:
        ...
        st.markdown("---")
        st.markdown("#### 👁️ Textförhandsgranskning (Rullningsbar box - redo för copy-paste):")
