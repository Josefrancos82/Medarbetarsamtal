...
# Safe import for Image and Generative AI
try:
...
# Custom Styling - Modern, dark background with glowing geometric lines & high contrast text
st.markdown("""
<style>
    /* Dark geometric art background */
    .stApp {
        background-color: #0B0F19;
        background-image: 
            radial-gradient(at 0% 0%, rgba(30, 58, 138, 0.4) 0px, transparent 50%),
            radial-gradient(at 100% 0%, rgba(124, 58, 237, 0.35) 0px, transparent 50%),
            radial-gradient(at 100% 100%, rgba(14, 165, 233, 0.3) 0px, transparent 50%),
            radial-gradient(at 0% 100%, rgba(236, 72, 153, 0.25) 0px, transparent 50%),
    ...
    /* Main Typography - Force White Text */
...
def call_gemini_with_fallback(contents):
    """
    Tries calling Gemini with dynamic model listing first to avoid 404 errors,
    then falls back across candidate model names.
    """
    if not (HAS_GENAI and GEMINI_CONFIGURED):
        return None

    candidate_names = []
    # 1. Try to query available models for this specific API key
    try:
    ...
    # 2. Hardcoded candidate names as fallback
    fallback_list = [
        'gemini-1.5-flash',
    ...
    all_candidates = []
    for c in candidate_names + fallback_list:
...
# ---------------------------------------------------------
def transform_notes_with_ai(raw_text, area_choice, uploaded_image=None):
    """Transforms raw notes or uploaded image into formal Swedish text for official school documentation."""
    prompt = f"""Du är en erfaren rektor och chef i svensk skola.
Omvandla följande råa eller handskrivna anteckningar från ett medarbetarsamtal till tjänstemannamässig, formell och professionell svenska lämplig för kommunens skoldokumentation.
...
5. Om en bild bifogas: läs av all handskriven eller tryckt text på bilden noggrant och omvandla hela innehållet till officiell text."""

    contents = [prompt]

    if uploaded_image is not None and HAS_PIL:
        try:
            img = Image.open(uploaded_image)
            contents.append(img)
        except Exception as img_err:
            st.error(f"Kunde inte läsa bildfilen: {img_err}")

    if raw_text and raw_text.strip():
        contents.append(f"Textanteckningar:\n{raw_text}")
    ...
    source_str = raw_text if raw_text else "Upplever periodvis hög belastning. God trivsel i laget. Utvecklar bildstöd i klassrummet."
...
def main():
    ...
    with st.sidebar:
        ...
        if GEMINI_CONFIGURED:
            ...
            if not HAS_GENAI:
                st.caption("ℹ️ Paket `google-generativeai` saknas.")
            elif not GEMINI_API_KEY:
                st.caption("ℹ️ Lägg till `GEMINI_API_KEY` i Streamlit Secrets.")
    ...
    with tab1:
        st.subheader("📝 Omvandla fotade eller råa anteckningar till officiell text")
        st.info("💡 Ladda upp ett foto på dina handskrivna anteckningar ELLER klistra in text. AI:n tolkar fotot direkt och skapar tjänstemannamässig svenska helt utan extra rubriker så att du kan klistra in texten direkt i dina Word-dokument.")

        col_input, col_output = st.columns([1, 1])

        with col_input:
            st.markdown("### 📥 Indata: Foto eller Text")
            ...
            with st.expander("✏️ Eller klistra in skriven text manuellt (Valfritt)"):
                raw_text_input = st.text_area(
                    "Skriven råtext:",
                    value="",
                    height=120,
                    placeholder="Skriv eller klistra in anteckningar här om du inte har något foto..."
                )
        ...
        with col_output:
            ...
            if btn_transform:
                if uploaded_file is None and not raw_text_input.strip():
                    st.warning("⚠️ Ladda upp ett foto ELLER skriv in text för att omvandla.")
                else:
                    with st.spinner("🧠 AI analyserar och omvandlar anteckningarna..."):
                        result_text = transform_notes_with_ai(
                            raw_text=raw_text_input,
                            area_choice=section_choice,
                            uploaded_image=uploaded_file
                        )
                    st.text_area("Kopiera texten nedan (Inga extra rubriker):", value=result_text, height=280)
                    st.success("✅ Texten är omvandlad och färdig att klistras in i din Word-mall!")
